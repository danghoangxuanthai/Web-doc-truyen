...
import {GET_GEN_MANGA} from "./QueryManga";
function Display() {
    const { loading, error, data } = useQuery(GET_GEN_MANGA);
    if(data === undefined){
        return (
            <Loading />
        )
    ...
    return (
        <div className={'flex flex-col justify-center items-center w-full bg-[#0b121a] '}>
            <h2 className={'max-w-[1000px] mx-auto pt-[3rem] text-[#999] font-bold'}>TRENDING MANGA</h2>
