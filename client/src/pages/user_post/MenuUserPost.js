...
export default function MenuUserPost({item, comment, onEdit, setOnEdit}) {
    const {auth} = useSelector(state => state)
    const dispatch = useDispatch()
    const handleDelete = () => {
        dispatch(deleteComment({item, comment, auth}))
    }
    ...
    return (
        <div className="w-56 text-right">
            ...
            <Menu as="div" className="relative inline-block text-left">
                ...
                >
                    <Menu.Items onClick={() => setOnEdit(true)} className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
